...
test-trees-as-requirements (unit: test/test-trees-as-requirements.bats)
  when a project uses contree
    ...
    and every tree names its coverage in parenthesised labelled pairs on the tree-name line, covering the categories src, unit, integration, functional, journey
...
pre-task-hook (src: hooks/session-start.sh; unit: test/pre-task-hook.bats; functional: test/functional/docker-entrypoint.sh)
  when a session starts
    ...
    and the agent is directed that every layer owns complete coverage of its own behaviour — a green higher-layer test never excuses a missing lower-layer test, and overlap across layers is intentional
...
post-task-hook (src: hooks/stop-drift-check.sh; unit: test/post-task-hook.bats; functional: test/functional/docker-entrypoint.sh)
  when Claude stops after a response that does not end with a question
    ...
    and a change-image nudge directs the agent to invoke gpt2 image to generate an image representing the change the task made
    and the change-image nudge directs the agent to choose what the image depicts from the nature of the change, its important details, and its intended audience
    and the change-image nudge directs the agent to surface those choices for the user to review
    and the change-image nudge directs the agent to treat a failure of gpt2 image as an error rather than proceeding as if it succeeded
...
change-writes-trees (src: skills/change/SKILL.md; unit: test/change-writes-trees.bats; functional: test/functional/docker-entrypoint.sh)
  ...
  when a tree is written
    then its coverage is named in parenthesised semicolon-separated pairs at the end of the tree-name line, labelled src / unit / integration / functional / journey
    and gaps are declared explicitly — "none" for expected-but-uncovered categories, omission for not-applicable ones
