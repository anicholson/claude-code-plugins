...
- **Test layers** — Journey (full user arc across capabilities and contexts, real everything, representative error paths, eventually succeeds), System (one capability wired whole-app with real driven adapters), Adapter (driving mocks app, driven hits real infra), Use-case (in-memory adapters), Domain (pure)
