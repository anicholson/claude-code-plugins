...
- **Outside-in** — start each capability from a failing System test at max realism; it pulls inner layers into being. Each discovered piece gets its own tree, TDD'd at its native layer with its own failing tests. The System test staying green is not the goal — completeness at every layer is. Inner trees own variations; System covers the user-visible arc, not combinatorial detail.
- **Layer completeness** — every layer owns complete coverage of its own behaviour at its own seam. A green higher-layer test never excuses a missing lower-layer test, and overlap across layers is intentional, not waste. When a System or other higher-layer test surfaces inner behaviour, that inner unit gets its own tree path and its own failing test before the code lands. Unit tests prove the unit is complete on its own terms; higher tests prove the wiring and the user-visible arc. "Already covered by X" is never a reason to skip Y.
