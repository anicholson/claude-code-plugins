...
- **Stop hook** — guard the contract. Fires after every response, detecting drift between intent and implementation. Yields the turn silently when the response ends with a question, so questions to the user aren't buried under drift-check output. Also emits a change-image nudge directing the agent to generate an image of the change with `gpt2 image` (on by default; `CONTREE_CHANGE_IMAGE=off` disables it).
...
Test trees are the living contract between intent and implementation — both the specification (in `TEST_TREES.md`) and the test structure (in code). They use EARS syntax to choose the right keyword for each requirement — `when` for events, `while` for state, `if/then` for errors, `where` for optional features, bare `then` for ubiquitous constraints. When one behaviour depends on another's outcome, it nests under that outcome (causal nesting) — not as a sibling. Contree prescribes **hexagonal architecture** with dependencies pointing inward and a strict linter (dependency-cruiser for JS/TS) enforcing the boundaries. Tests are layered outermost-in: a **Journey** (the full user arc across capabilities and the contexts it establishes, at max realism — real everything, representative error paths, eventually succeeds), then **System** (one capability wired whole-app with real driven adapters), then the hex inner layers **Adapter** (driving and driven), **Use-case**, and **Domain**. Journey and System are both max-validity functional testing; the **Journey is the outside-in entry point** and System tests sit interior to it. When breadth at max realism is unaffordable, lean on the journey and push combinatorial detail to inner layers, never to broad in-memory wiring. Each driven port ships with an in-memory twin so Use-case tests run fast; a shared port contract suite is imported by both the in-memory adapter's test file and the real adapter's test file, making substitution sound. Outside-in TDD starts from a failing Journey test; System and inner-layer trees and tests exist only because the failing journey/functional test cannot be satisfied without them. **Journey and functional coverage is never coverage of the layers beneath: implementation waits for a ground-level failing test under the journey/functional failure** — "already covered above" is the signal to write the unit's own failing test, not to skip it. Every behavioural unit gets its own tree in `TEST_TREES.md`; every tree reifies exactly one test file. See `skills/tdd/SKILL.md` for the full framing. `change` sets expected behaviour. `sync` identifies where reality has drifted. `tdd` closes gaps. The stop hook guards the contract — drift is never resolved silently. Coding rules enforce principles (simplicity, expressiveness, fail-fast) across all work.
