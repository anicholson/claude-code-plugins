...
- **trunk-sync** — multi-agent sync hook + seance CLI. The per-edit hook runs on Claude Code and Codex (stdin hook via `scripts/trunk-sync.sh`) and on OpenCode (the `src/opencode-plugin.ts` plugin shipped under `.opencode/`, which subprocesses the same `hook-entry`). npm package `@anicholson/trunk-sync` (republished under `@anicholson` until the OpenCode work is upstreamed to `@elimydlarz`).
