...
Each script checks for clean source, runs tests (if any), bumps the version, commits, tags, pushes, and creates a GitHub release whose notes are generated by `scripts/release-notes.sh` (drops version-bump / `build:` / "trunk sync noise" commits, collapses repeated `auto(<id>): …` commits from one trunk-sync session into a single line, strips the `auto(...)` prefix). trunk-sync also publishes to npm.
